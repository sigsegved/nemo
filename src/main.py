...
if Counter is not None:
    ...
    circuit_breaker_active = Gauge(
        "nemo_circuit_breaker_active", "Circuit breaker status (1=active, 0=inactive)"
    )
    signal_processing_time = Histogram(
        "nemo_signal_processing_seconds", "Time spent processing signals"
    )
    health_check_status = Gauge(
        "nemo_health_check_status", "Health check status (1=healthy, 0=unhealthy)"
    )
else:
    trade_signals_total = None
    positions_active = None
    circuit_breaker_active = None
    signal_processing_time = None
    ...
    async def _execute_signal(self, signal: TradeSignal) -> None:
        ...
        try:
            ...
            if self.enable_paper_trading:
                ...
                self.logger.info(
                    "Paper trade executed", status=status, signal_id=id(signal)
                )
            else:
                ...
                self.logger.warning(
                    "Live trading not implemented", signal=asdict(signal)
                )
            ...
            self.logger.error(
                "Error executing signal", signal=asdict(signal), error=str(e)
            )
            self.error_count += 1
        ...
        while not self.shutdown_event.is_set():
            try:
                ...
                for symbol in self.symbols:
                    ...
                    trigger_engine = self.trigger_engines[symbol]
                    trigger_engine.process_trade(price, volume, timestamp)
...
async def main_async(config_path: str, trading_mode: str) -> None:
    ...
    logger.info(
        "Starting Nemo Volatility Harvest Bot",
    ...
    if start_http_server is not None:
        ...
        logger.info("Prometheus metrics disabled (prometheus_client not available)")
    ...
    try:
        ...
        logger.error(
            "Fatal error in main loop", error=str(e), traceback=traceback.format_exc()
        )
        raise
    finally:
