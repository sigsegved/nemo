...
from .vwap import VWAPCalculator, VolumeAggregator
...
class TriggerSignal:
    ...
    def __init__(self, trigger_type: TriggerType, strength: Decimal, 
                 timestamp: datetime, symbol: str, metadata: Optional[Dict] = None):
        """
    ...
    def __repr__(self):
        return (f"TriggerSignal({self.trigger_type.value}, "
...
class PriceDeviationTrigger:
    ...
    def add_trade(self, price: Union[Decimal, float], volume: Union[Decimal, float], 
                  timestamp: datetime) -> None:
        """Add trade data to VWAP calculator."""
        self.vwap_calculator.add_trade(price, volume, timestamp)
    
    def check_trigger(self, current_price: Union[Decimal, float], 
                      symbol: str, timestamp: datetime) -> Optional[TriggerSignal]:
        """
        ...
        if (self.last_signal_time and 
            (timestamp - self.last_signal_time).total_seconds() < self.cooldown_seconds):
            return None
        ...
        if abs_deviation >= self.threshold:
            ...
            strength = min(abs_deviation / self.threshold, Decimal('2.0')) / Decimal('2.0')
...
class VolumeSpikeTrigger:
    ...
    def __init__(self, spike_multiplier: Decimal = Decimal('3.0'),
                 window_minutes: int = 3, lookback_periods: int = 10):
        """
    ...
    def check_trigger(self, symbol: str, timestamp: datetime) -> Optional[TriggerSignal]:
        """
        ...
        if volume_ratio >= self.spike_multiplier:
            ...
            strength = min(volume_ratio / self.spike_multiplier, Decimal('2.0')) / Decimal('2.0')
...
class LiquidationTracker:
    ...
    def __init__(self, window_minutes: int = 3, min_liquidation_sum: Decimal = Decimal('100000')):
        """
    ...
    def add_liquidation(self, liquidation_value: Union[Decimal, float], 
                       timestamp: datetime) -> None:
        """
    ...
    def _clean_old_liquidations(self, current_time: datetime) -> None:
        ...
        while (self.liquidations and 
               self.liquidations[0]['timestamp'] < cutoff_time):
    ...
    def check_trigger(self, symbol: str, timestamp: datetime) -> Optional[TriggerSignal]:
        ...
        if liquidation_sum >= self.min_liquidation_sum:
            # Calculate signal strength
            strength = min(liquidation_sum / self.min_liquidation_sum, Decimal('2.0')) / Decimal('2.0')
...
class TriggerEngine:
    ...
    def process_trade(self, price: Union[Decimal, float], volume: Union[Decimal, float], 
                     timestamp: datetime) -> List[TriggerSignal]:
        """
        ...
        liquidation_signal = self.liquidation_tracker.check_trigger(self.symbol, timestamp)
    ...
    def process_liquidation(self, liquidation_value: Union[Decimal, float], 
                           timestamp: datetime) -> Optional[TriggerSignal]:
        """
