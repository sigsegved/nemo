...
from strategy.trigger import (
    TriggerType, TriggerSignal, PriceDeviationTrigger, VolumeSpikeTrigger,
    LiquidationTracker, TriggerEngine
)
...
class TestLiquidationTracker:
    ...
    def setup_method(self):
        ...
        self.tracker = LiquidationTracker(
            window_minutes=3,
            min_liquidation_sum=Decimal('100000')
        )
        self.base_time = datetime(2024, 1, 1, 12, 0, 0)
        self.symbol = "BTCUSD"
    ...
    def test_liquidation_sum_calculation(self):
        ...
        self.tracker.add_liquidation(Decimal('30000'), self.base_time + timedelta(minutes=1))
        self.tracker.add_liquidation(Decimal('25000'), self.base_time + timedelta(minutes=2))
        
        total = self.tracker.get_liquidation_sum(self.base_time + timedelta(minutes=2, seconds=30))
    ...
    def test_liquidation_window_filtering(self):
        ...
        self.tracker.add_liquidation(Decimal('45000'), self.base_time + timedelta(minutes=1))
    ...
    def test_liquidation_trigger_above_threshold(self):
        ...
        self.tracker.add_liquidation(Decimal('50000'), self.base_time + timedelta(minutes=1))
        
        signal = self.tracker.check_trigger(self.symbol, self.base_time + timedelta(minutes=2))
...
class TestTriggerEngine:
    ...
    def test_process_trade_single_trigger(self):
        ...
        price_signals = [s for s in signals if s.trigger_type == TriggerType.PRICE_DEVIATION]
    ...
    def test_process_trade_multiple_triggers(self):
        ...
        for i in range(5):
            self.engine.volume_spike_trigger.add_volume(
                Decimal('1000'), 
                self.base_time - timedelta(minutes=3*(i+1))
            )
    ...
    def test_process_liquidation(self):
        ...
        signal = self.engine.process_liquidation(
            Decimal('150000'), self.base_time
        )
    ...
    def test_signal_history_management(self):
        ...
        for i in range(3):
            self.engine.process_liquidation(
                Decimal('150000'), 
                self.base_time + timedelta(minutes=(i+10)*4)
            )
    ...
    def test_realistic_trading_scenario(self):
        ...
        for i in range(10):
            engine.volume_spike_trigger.add_volume(
                Decimal('1000'), 
                base_time - timedelta(minutes=3*(i+1))
            )
