...
class TestVolumeAggregator:
    ...
    def test_volume_aggregation(self):
        ...
        self.aggregator.add_volume(
            Decimal("200"), self.base_time + timedelta(minutes=1)
        )
        self.aggregator.add_volume(
            Decimal("150"), self.base_time + timedelta(minutes=2)
        )

        total_volume = self.aggregator.get_total_volume(
    ...
    def test_average_volume_calculation(self):
        ...
        for i in range(5):
            ...
            self.aggregator.add_volume(
                Decimal("200"), period_start + timedelta(minutes=1)
            )
