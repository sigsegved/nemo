...
from src.main import TradingOrchestrator, load_config, setup_logging
...
class TestTradingOrchestrator:
    ...
    async def test_signal_execution(self):
        ...
        from src.strategy.risk import PositionSide, StrategyType, TradeSignal
    ...
    async def test_flatten_positions_on_shutdown(self):
        ...
        from src.strategy.risk import Position, PositionSide, StrategyType
...
class TestIntegration:
    ...
    async def test_end_to_end_trading_cycle(self):
        ...
        orchestrator.vwap_calculators["BTCUSD"].get_vwap("3min")
...
class TestMetricsAndMonitoring:
    ...
    def test_metrics_initialization(self):
        """Test that Prometheus metrics are initialized or None if unavailable."""
        ...
        if prometheus_available:
            # Metrics should be initialized when prometheus_client is available
            ...
            # Metrics should be None when prometheus_client is not available
            assert trade_signals_total is None
            assert positions_active is None
            assert circuit_breaker_active is None
            assert signal_processing_time is None
            assert health_check_status is None
